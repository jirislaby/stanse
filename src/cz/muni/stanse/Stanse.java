 ...
 * functionality is command-line parsing.
 *
 * @version $Id$
 */
public final class Stanse {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ClassLogger.error(Stanse.class, "can't parse commandline: " + e.getLocalizedMessage());
            return;
        }
    ...
    public synchronized void dumpAST() {
        ...
        for (final Unit unit : getConfiguration().getSourceConfiguration().getLazySourceInternals().getUnits()) {
            final String name = getOutputDirectory().toString().isEmpty() ? unit.getName() : (new File(unit.getName())).getName();
            File xmlFile = new File(getOutputDirectory(), name + ".xml");
            try {
                XMLAlgo.outputXML(getUnitManager().getXMLDocument(unit), new PrintStream(xmlFile));
    ...
    public synchronized void dumpCFG() {
        assert (getConfiguration() != null);
        for (final CFGHandle cfg : getConfiguration().getSourceConfiguration().getLazySourceInternals().getCFGHandles()) {
            final String unitName = getUnitManager().getUnitName(cfg);
            final String name = getOutputDirectory().toString().isEmpty() ? unitName : (new File(unitName)).getName();
            final File cfgFile = new File(getOutputDirectory(), name + "." + cfg.getFunctionName() + ".dot");
            try {
    ...
    public synchronized void dumpCallGraph() {
        ...
        try {
            ...
            out.write(cz.muni.stanse.utils.CallGraphToDot.run(getConfiguration().getSourceConfiguration().getLazySourceInternals().getCallGraph()));
            out.close();
        } catch (IOException exception) {
            exception.printStackTrace();
        }
    ...
    private static void buildConfiguration(final CmdLineManager cmdLineManager) {
        getInstance().setConfiguration(new Configuration(cmdLineManager.getSourceConfiguration(), cmdLineManager.getCheckerConfiguration()));
    }
    ...
    private static void setOutputDirectory(final CmdLineManager cmdLineManager) {
        ...
        if (!outDir.exists()) {
            System.out.print("Output directory: '" + outDirName + "' does not exist.\n\tCreating it...");
            if (outDir.mkdirs())
    ...
    private static void processDumping(final CmdLineManager cmdLineManager) {
        if (getInstance().getConfiguration() == null)
            return;
        if (cmdLineManager.dumpAST())
            getInstance().dumpAST();
        if (cmdLineManager.dumpCFG())
            getInstance().dumpCFG();
        if (cmdLineManager.dumpCallGraph())
            getInstance().dumpCallGraph();
    }
    ...
    private static void processStats(final CmdLineManager cmdLineManager) {
        ...
        if (buildFile != null) {
            System.out.print("Statistical results will be stored in output " + "file:\n   " + buildFile + "\n\n\n");
        ...
        if (databaseFile == null || !new File(databaseFile).exists()) {
            System.err.print("Error: cannot find statistical database " + "XML file:\n   " + databaseFile + "\n\n\n");
            return;
        }
        final org.dom4j.Document database = cz.muni.stanse.statistics.StatisticalDatabaseLoader.run(databaseFile);
        if (database == null) {
            System.err.print("Error: parsing of content of database " + "XML file has FAILED. Parsed file was:\n   " + databaseFile + "\n\n\n");
            return;
        }

        final String orderingDir = cmdLineManager.statsOrderingRootDir();
        if (orderingDir != null) {
            cz.muni.stanse.statistics.CheckerErrorsSorter.run(database, cmdLineManager.getStatsOrdering(), orderingDir);
            return;
        }
        ...
        if (cmdLineManager.doStatsGuiTracing()) {
            ...
            if (out == null) {
                ClassLogger.error(Stanse.class, "wrong gui-tracing " + "commandline");
                return;
            }
            ...
            if (origDir == null || newDir == null)
                ...
                CheckerErrorsGuiTracing.run(database, Pair.make(origDir, newDir), out);
            return;
        }
        ...
        if (cmdLineManager.doStatsMerge()) {
            ...
            if (root == null || out == null) {
                ClassLogger.error(Stanse.class, "wrong stats-merge " + "commandline");
                return;
            }
        ...
        if (cmdLineManager.doStatsPerformance()) {
            ...
            if (outFile == null || outFormat == null) {
                ClassLogger.error(Stanse.class, "wrong performance " + "commandline");
            } else
                PerformanceDataBuilder.run(database, outFile, outFormat);
        }
        ...
        if (cmdLineManager.doStatsReports()) {
            ...
            if (outFile == null || outFormat == null) {
                ClassLogger.error(Stanse.class, "wrong reports " + "commandline");
            } else
    ...
    private static void startTUI(final String style) {
        ...
        final java.util.Vector<CheckerError> errors = new java.util.Vector<CheckerError>();
        getInstance().getConfiguration().evaluate_EachUnitSeparately(new CheckerErrorReceiver() {
            @Override
    ...
    private static String buildRootDirectory() {
        ...
        if (rootDirectory == null) {
            ...
            try {
                rootDirectory = new File(ClassLocation.get("cz.muni.stanse.Stanse")).getParent();
