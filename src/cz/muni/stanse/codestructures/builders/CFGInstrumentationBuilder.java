 ...
 */
public final class CFGInstrumentationBuilder {
     ...
     */
    public static Pair< HashMap<CFG,CFG>,HashMap<CFGNode,CFGNode> >
    run(final Collection<CFG> CFGs) {
     ...
     */
    public static Pair< CFG,HashMap<CFGNode,CFGNode> >
    ...
    private static void convertBranchesConditionsToAssertions(
        ...
        for (CFGNode node : forwardMapping.keySet()) {
            if (!(node instanceof CFGBranchNode))
                continue;
            CFGNode nodeCopy = forwardMapping.get(node);

            // TODO: following block is related to problem with missing
            //       entry node in current implementation of CFG. The problem is
            //       that start node in original CFG can be branch-node. Thus
            //       copy of branch node is then first follower of nodeCopy.
            //       When original CFG supports entry node, then following block
            //       can be removed.
            {
                if (!nodeCopy.getElement().getName().equals("branch"))
                    nodeCopy = nodeCopy.getSuccessors().get(0);
            }

            addAssertions(nodeCopy,(CFGBranchNode)node,
