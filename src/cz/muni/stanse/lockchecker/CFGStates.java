...
public class CFGStates {
	 ...
	 */
	public CFGStates(CFGNode startNode, State enterState, ErrorHolder errHandler, Configuration conf) {
	...
	public boolean propagate(CFGNode from, CFGNode to) {
		LockError res = null;
		if(!cfgStates.containsKey(to)) cfgStates.put(to, new State());
		...
		for(XMLPattern pattern : conf.getLocks()) {			
			...
			if(pair.getFirst()) {
				...
				res = State.checkDoubles(false, VarTransformations.makeArgument(pair.getSecond().getVarsMap().get("A")), fromState);
				fromState = State.transformState(false, VarTransformations.makeArgument(pair.getSecond().getVarsMap().get("A")), fromState);
				break;
			}
		}
		...
		for(XMLPattern pattern : conf.getUnlocks()) {
			...
			if(pair.getFirst()) {
				...
				res = State.checkDoubles(true, VarTransformations.makeArgument(pair.getSecond().getVarsMap().get("A")), fromState);
				fromState = State.transformState(true, VarTransformations.makeArgument(pair.getSecond().getVarsMap().get("A")), fromState);
				break;
			}	
		}
	...
	public String toString() {
		...
		boolean first = true;
		for(Entry<CFGNode, State> entry : this.cfgStates.entrySet()) {
			if(first) {
				first = false;				
			} else {
				sb.append("\n");
